...
export function generateStaticParams(){return articles.filter(a=>a.kind==="effect").map(a=>({slug:a.slug}))}
...
export default async function Page({params}:{params:Promise<{slug:string}>}){const a=getArticle((await params).slug);if(!a||a.kind!=="effect")notFound();return <ArticlePage article={a}/>}
